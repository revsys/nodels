...
def test_name(node1):
    assert node1.get_name() == "ip-10-131-10-138.us-west-2.compute.internal"


def test_external_id(node1):
    assert node1.get_external_id() == "i-09587a5ad7a5d60f3"


def test_created(node1):
    assert isinstance(node1.get_created(), datetime.datetime)


def test_size(node1):
    assert node1.get_size() == "r5.xlarge"
